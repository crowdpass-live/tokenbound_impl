...
export async function detectAvailableWalletProviders(): Promise<
  WalletProviderInfo[]
> {
  ...
  if (freighterInstalled) {
    try {
      ...
      if (window.freighter && window.freighter.isConnected) {
        ...
        window.stellar.freighter.isConnected
      ) {
    ...
    case "freighter": {
      ...
      try {
        ...
        if (freighter.requestAccess) {
          ...
          throw new Error(
            "Freighter API not available. Please update your Freighter extension.",
          );
        }
        ...
        throw new Error(
          response?.error ||
            "Failed to connect to Freighter. Please ensure you have unlocked Freighter.",
        );
      } catch (error: any) {
        console.error("Freighter connection error:", error);
        throw new Error(
          error.message ||
            "Failed to connect to Freighter. Please check if Freighter is unlocked.",
        );
      }
    }
