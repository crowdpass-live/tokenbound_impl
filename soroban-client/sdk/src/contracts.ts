...
export class EventManagerContract extends BaseContract {
  ...
  markAttendance(
    ...
    return this.write(
      ...
      [
        nativeToScVal(eventId, { type: "u32" }),
        nativeToScVal(ticketTokenId, { type: "u128" }),
      ],
      options,
    );
  }
  ...
  distributePoaps(
    input: {
      ...
      readonly poapContract: string;
      readonly ticketTokenIds: readonly bigint[];
      readonly tbaRecipients: readonly string[];
    ...
    return this.write(
      ...
      [
        nativeToScVal(input.eventId, { type: "u32" }),
        nativeToScVal(input.poapContract, { type: "address" }),
        nativeToScVal(
          input.ticketTokenIds.map((id) => nativeToScVal(id, { type: "u128" })),
          { type: "vec" },
        ),
        nativeToScVal(
          input.tbaRecipients.map((a) => nativeToScVal(a, { type: "address" })),
          { type: "vec" },
        ),
...
export class PoapNftContract extends BaseContract {
  constructor(core: SorobanSdkCore) {
    super(core, "poapNft");
  }
  ...
  batchMintPoap(
    ...
    return this.write(
      ...
      [
        nativeToScVal(
          recipients.map((a) => nativeToScVal(a, { type: "address" })),
          { type: "vec" },
        ),
        nativeToScVal(eventId, { type: "u32" }),
        nativeToScVal(metadataUri, { type: "string" }),
  ...
  async tokenMetadata(tokenId: bigint, options?: InvokeOptions) {
    const raw = await this.read<Record<string, unknown>>(
      "token_metadata",
      [nativeToScVal(tokenId, { type: "u128" })],
      options,
  ...
  hasClaimed(eventId: number, recipient: string, options?: InvokeOptions) {
    return this.read<boolean>(
      "has_claimed",
      [
        nativeToScVal(eventId, { type: "u32" }),
